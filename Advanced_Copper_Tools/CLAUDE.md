...
- **But a BLUEPRINT result CAN carry SD4 with no C# at all, and that is how the Bronze Sheet chain works.** Put the Special4 bit in the ingredient's `AffectResultDurabilities` mask (`MaskValue: 448` = SD2|SD3|SD4) and the consumed bar's Metal Type is averaged onto the product. This is a different mechanism from `ProducedCards`/`OnFull`, so the rule above does not forbid it. **`BlueprintResult[].TransferRules` is NOT that mechanism**: it is a `TransferToDrop` whose real fields are the `Special1..4` enum, so ACT's `TransferSpecial2/3/4: true` keys in `Bp_MetalSheet.json`, `Bp_BronzeSheet.json` and `Bp_CastStoveTop.json` bind to nothing and are vestigial - harmless only because mask 448 already does the work, so never delete the mask on the belief those keys cover it. Full mechanism + the decompiled call chain: root `CLAUDE.md` §Blueprint Result Durability Transfer. See memory: `reference_blueprint_durability_transfer_mask`.
